...
func (handler DriverHandler) FinishedListOrders(c echo.Context) error {
	...
	links["self"] = configs.Get().App.BaseURL + "/api/drivers/history_orders?page=" + strconv.Itoa(page)
	...
	filters := []map[string]interface{}{
		{
			"field":    "driver_id",
			"operator": "=",
			"value":    strconv.Itoa(int(driver.ID)),
		},
		{
			"field":    "status",
			"operator": "=",
